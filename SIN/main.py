...
def getMispl(state):
    ...
    for i in range(len(state)):
        if state[i]!=0 and state[i] != end_state[i]:
            total+=1
    return total
...
def seq(state):
    ...
    for i in range(len(ord)):
        if int(state[ord[i]]) == 0:
            continue
        if int(state[ord[(i+1)%len(ord)]]) != int(state[ord[i]]) % 8 + 1:
            cost+=6
...
import numpy as np
def h4(state):
    per_row = np.array([[1,2,3],[8,0,4],[7,6,5]])
    per_col = per_row.T
    state_array = []
    for i in range(0,9,3):
        state_array.append([int(j) for j in state[i:i+3]])
     
    cost = 0
    for per_r, r in zip(per_row, state_array):
        cost += len(set(per_r)-set(r))
    for per_r, r in zip(per_col, np.array(state_array).T):
        cost += len(set(per_r)-set(r))
