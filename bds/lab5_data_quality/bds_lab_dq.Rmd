...
We have many missing values in Apgar score and some in weight column. Other features are complete. It means 95.12% of our data is complete. It's quite high score. However, the number of missing values for Apgar score may exclude this feature from prediction further. However, it seems that not all values are considered. E.g. for anestestesia type we have 135 values with empty string, which should probably be treated as a null value as well.
...
The incompleteness of the data comes mostly from missing Apgar scores. There are also some weights missing. The incompletness is fluctuant and we can't see any pattern in misses. However, as mentioned in previous task, some of the null values aren't recognized as such.
...
![](images/Zrzut ekranu z 2023-10-25 13-48-12.png)
